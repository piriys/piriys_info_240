...
BEGIN TRANSACTION AddPromotion
		...
		BEGIN
			...
			ELSE
				THROW 51336, 'Discount is not existed in database. Please add new discount by using uspAddDiscount', 1
		END
...
BEGIN TRANSACTION AddDisk
	IF NOT EXISTS ( 
		...
		THROW 51337, 'Movie is not existed in database. Please add new movie by using uspAddMovie', 1
	...
	BEGIN
		...
		ELSE
			THROW 51339, 'Format is not existed in database. Please add new format by using uspAddFormat', 1				
	END	
...
BEGIN TRANSACTION AddConditionToDisc
	...
	IF NOT EXISTS (
		...
		THROW 51338, 'Disc is not existed in database. Please add new disc by using uspAddDisc', 1	
	...
	BEGIN
		...
		ELSE
			THROW 51338, 'Condition is not existed in database. Please add new condition by using uspAddCondition', 1	
	END	
...
BEGIN TRANSACTION AddGenreToMovie
	...
	BEGIN
		...
		ELSE 
			THROW 51341, 'Genre is not existed in database. Please add new genre by using uspAddGenre', 1
	END
...
BEGIN TRANSACTION AddStatusToMovie
	...
	BEGIN
		...
		ELSE 
			THROW 51342, 'Status is not existed in database. Please add new status by using uspAddStatus', 1
	END
...
BEGIN TRANSACTION AddMembership
	...
	BEGIN
		...
		ELSE
			THROW 51343, 'Membership type is not existed in database. Please add new membership type by using uspAddMembershipType', 1
	END
...
BEGIN TRANSACTION AddRental
	...
	IF NOT EXISTS (
		...
		THROW 51344, 'Customer is not existed in database. Please add new customer by using uspAddCustomer', 1
	...
	IF NOT EXISTS (
		...
		THROW 51345, 'Promotion is not existed in database. Please add new promotion by using uspAddPromotion', 1
...
BEGIN TRANSACTION AddCast
	...
	BEGIN
		...
		ELSE
			THROW 51346, 'Professional is not existed in database. Please add new professional by using uspAddProfessional',1
	END
...
BEGIN TRANSACTION AddNomination
	...
	BEGIN 
		...
		ELSE
			THROW 51348, 'Award is not existed in database. Please add new award by using uspAddAward', 1
	END
	...
	BEGIN
		...
		ELSE
			THROW 51347, 'Cast is not existed in database. Please add new cast by using uspAddCast', 1
...
CREATE PROCEDURE uspAddWriter
	...
	@releaseDate DATE,
	----Actor Attribute
	@draftSubmitDate DATE,
	@revisionRetainer BIT = 0,
	----Additional Flag
	@createChildIfNotExisted BIT = 0
AS
